...
mod test {
    ...
    fn test_winternitz() {
        ...
        println!("winternitz sign_hash script size: {:} bytes", script.len());
    ...
    fn test_stack_optimized_winternitz() {
        ...
        println!(
            "Stack Optimized Winternitz signature size:\n \t{:?} bytes / {:?} bits \n\t{:?} bytes / bit \n\t@ Max stack usage of {} elements",
            script.len(),
            N0 * 4,
            script.len() as f64 / (N0 * 4) as f64,
            execute_script(script.clone()).stats.max_nb_stack_items
        ...
        println!(
            "Stack Optimized winternitz sign_hash script size: {:} bytes",
            script.len()
        );
